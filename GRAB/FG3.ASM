...
txt	db	13,10									       
	...
	db	"º   Future-Multi-Grabber V1.2     Copyright (C) 1991 ÀÅÙ / The Future Crew    º",13,10
	db	"ÇÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¶",13,10
	db	"º Press the mouse MIDDLE-button to grab a Deluxepaint LBM (PIC?.LBM).         º",13,10
	db	"º Hold MIDDLEb and press all others to save memory dump (MEMORY.DAT).         º",13,10
	db	"º Hold MIDDLEb and click the LEFTb to enter the scroll mode.                  º",13,10
	db	"º Hold MIDDLEb and click the RIGHTb to enter menu mode.                       º",13,10
	db	"ÇÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¶",13,10
...
txt3	db	13,10,'WARNING: MouseDriver (3 button mouse required) not found!',13,10,'$'
INTER	equ	33h
INTER2	equ	28h
INTER3	equ	8h
INTER4	equ	10h
...
deflbm4	LABEL BYTE ;main header for 320x400
	db	70,79,82,77, 0,0,0,0 ,73,76,66,77,66,77,72,68
			;    ^filelength-8
	db	0,0,0,20,1,64,1,144,0,0,0,0,8,0,1,0
	db	0,255,1,1,1,64,0,200,67,77,65,80,0,0,3,0
...
savelbmdata4 PROC NEAR
	mov	bx,cs:handle
	mov	ah,40h
	mov	dx,OFFSET deflbm4
	mov	cx,30h
	int	21h
	mov	bx,cs:handle
	mov	ah,40h
